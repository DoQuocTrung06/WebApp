...
import { 
  ...
  MdPersonAdd // [CẬP NHẬT TÍNH NĂNG SHARE] Import icon
} from "react-icons/md";
...
// [CẬP NHẬT TÍNH NĂNG SHARE] Lùi 1 thư mục để import ShareModal vì đang ở trong thư mục RichNoteModal
import ShareModal from '../ShareModal'; 

function RichNoteModal({ 
  isOpen, onClose, initialData, onSave, onDelete, onRestore, 
  theme, activeTab, onArchive, availableLabels = []
}) {
  ...
  // [CẬP NHẬT TÍNH NĂNG SHARE] State cho ShareModal
    ...
    <>
      ...
      >
        ...
        <div 
          ...
          style={{ 
            width: '600px', maxWidth: '95vw', borderRadius: '12px',
            display: showShareModal ? 'none' : 'block' // <--- CHÌA KHÓA LÀ Ở ĐÂY
          }} 
        ...
        >
          ...
          <div className="p-3">
            <div className="d-flex flex-wrap align-items-center mb-2" style={{ padding: '6px 12px' }}>
              {reminder && (
                <div className={`badge rounded-pill border ${theme === 'dark' ? 'bg-dark text-white' : 'bg-light text-dark'}`} style={{ padding: '6px 12px' }}>
              ...
              {labels.map(lbl => (
                <div key={lbl} className={`badge rounded-pill border ${theme === 'dark' ? 'bg-dark text-white' : 'bg-light text-dark'}`} style={{ padding: '6px 12px' }}>
                  {lbl}
                  {!isTrash && <MdClose className="ms-1 cursor-pointer" onClick={() => setLabels(labels.filter(l => l !== lbl))} />}
                </div>
              ))}

              {/* 3. AVATAR NGƯỜI CỘNG TÁC */}
          ...
          <div className="d-flex align-items-center justify-content-between p-2 px-3 border-top" ref={menuContainerRef}>
            {isTrash ? (
              ...
              <div className="d-flex gap-1 position-relative">
                ...
                <button 
                  className="btn btn-sm text-muted hover-bg-light rounded-circle" 
                  onClick={() => setShowShareModal(true)} 
                  title="Cộng tác viên"
                >
                  <MdPersonAdd size={20} />
        ...
        {/* Nếu showShareModal là true thì nó sẽ hiện ra thay thế cho ghi chú */}
        {showShareModal && (
          <ShareModal 
            // Truyền danh sách mail hiện tại vào để ShareModal biết
            currentCollaborators={collaborators} 
            
            // Khi bấm Hủy ở ShareModal
            onClose={() => setShowShareModal(false)} 
            
            // Khi bấm Lưu ở ShareModal
            onSave={(newCollaboratorsList) => {
              setCollaborators(newCollaboratorsList); // Lưu danh sách mail mới vào state
              setShowShareModal(false); // Tắt ShareModal -> Ghi chú tự động hiện lại với Avatar mới
            }}
          />
        )}
      </div>
